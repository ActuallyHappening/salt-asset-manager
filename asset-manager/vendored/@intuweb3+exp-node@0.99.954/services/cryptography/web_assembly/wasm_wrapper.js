import init, { create_js_seed, pre_register, form_transaction, create_random_message, encrypt_data, decrypt_data, compute_pedersen_dealing, compute_pedersen_opening, compute_simple_dealing, compute_simple_opening, compute_multiply_dealing, compute_multiply_opening, user_sign_message, user_sign_message_without_lambda, combine_signed_shares, combine_signed_shares_without_lambda, parse_transaction, compute_simple_dealing_reshare, generate_master_public_key_and_address, compute_simple_opening_reshare_once_or_twice, combine_signed_shares_reshare, compute_multiply_opening_reshare, user_sign_message_reshare, get_polybase_key, get_unique_hash, } from "./intu_crypto";
//
export async function _createSeed() {
    // this is essentially vault preregistration, called before creating smart contract
    const seed_result = await init().then(() => create_js_seed());
    return { seed: seed_result };
}
export async function _createRandomMessage() {
    const randomMessage = await init().then(() => create_random_message());
    return { message: randomMessage };
}
export async function _getPolybaseKey(signature) {
    let pbk = await init().then(() => get_polybase_key(signature));
    return { key: pbk };
}
export async function _getUniqueHash(signature) {
    let did = await init().then(() => get_unique_hash(signature, 0));
    return { key: did };
}
export async function _preRegister(signature) {
    let pre_reg = await init().then(() => pre_register(signature));
    return { encryptionKey: pre_reg[0], megaPublicKey: pre_reg[1], encMegaSecretKey: pre_reg[2] };
}
export async function _encryptData(encryptionKey, dataToEncrypt) {
    return await init().then(() => encrypt_data(encryptionKey, dataToEncrypt));
}
export async function _decryptData(encryptionSignature, dataToDecrypt) {
    return await init().then(() => decrypt_data(encryptionSignature, dataToDecrypt, 0));
}
export async function _formTransaction(to, value, chainId, nonce, data, gasPrice, gas, decimal) {
    return await init().then(() => form_transaction(to, value, chainId, nonce, data, gasPrice, gas, decimal));
}
export async function _parseTransaction(txdata) {
    let parsedTransaction = await init().then(() => parse_transaction(txdata));
    return {
        to: parsedTransaction[0],
        value: String(parsedTransaction[1]),
...
export async function _registerStepOne(seed, threshold, index, megaPkArray) {
    //dealing to be added to pedersen dealing array.
    const dealingArray = new Array();
    await init().then(() => {
        dealingArray.push(compute_pedersen_dealing(seed, threshold, index, megaPkArray));
        dealingArray.push(compute_pedersen_dealing(seed, threshold, index, megaPkArray));
        dealingArray.push(compute_pedersen_dealing(seed, threshold, index, megaPkArray));
    });
...
export async function _registerStepTwo(seed, threshold, index, megaPkArray, encryptionSignature, encryptedMegaSecret, dealingsKeyArray, dealingsKappaArray, dealingsLambdaArray) {
    ...
    await init().then(() => {
        let opening1Array = compute_pedersen_opening(seed, threshold, index, encryptionSignature, encryptedMegaSecret, dealingsKeyArray);
        let opening2Array = compute_pedersen_opening(seed, threshold, index, encryptionSignature, encryptedMegaSecret, dealingsKappaArray);
        let opening3Array = compute_pedersen_opening(seed, threshold, index, encryptionSignature, encryptedMegaSecret, dealingsLambdaArray);
        ...
        transcriptArray.push(opening3Array[1]);
        dealingArray.push(compute_simple_dealing(seed, threshold, index, encryptionSignature, megaPkArray, openingArray[0]));
        dealingArray.push(compute_simple_dealing(seed, threshold, index, encryptionSignature, megaPkArray, openingArray[1]));
    });
    return { pedersenOpeningArray: openingArray, simpleDealingArray: dealingArray, pedersenTranscriptArray: transcriptArray };
}
export async function _registerStepThree(seed, threshold, index, megaPkArray, encryptionSignature, encryptedMegaSecret, simpleDealingKeyArray, // step1
simpleDealingKappaArray, // step1
pedersenTranscriptKey, // step 2
pedersenTranscriptKappa, // step 2
pedersenOpeningLambda // step 2
) {
    const openingArray = new Array();
    const dealingArray = new Array();
    const transcriptArray = new Array();
    await init().then(() => {
        let opening1Array = compute_simple_opening(seed, threshold, index, encryptionSignature, encryptedMegaSecret, simpleDealingKeyArray, pedersenTranscriptKey);
        let opening2Array = compute_simple_opening(seed, threshold, index, encryptionSignature, encryptedMegaSecret, simpleDealingKappaArray, pedersenTranscriptKappa);
        openingArray.push(opening1Array[0]);
        openingArray.push(opening2Array[0]);
        transcriptArray.push(opening1Array[1]);
        transcriptArray.push(opening2Array[1]);
        dealingArray.push(compute_multiply_dealing(seed, threshold, index, encryptionSignature, megaPkArray, openingArray[0], pedersenOpeningLambda));
        dealingArray.push(compute_multiply_dealing(seed, threshold, index, encryptionSignature, megaPkArray, openingArray[1], pedersenOpeningLambda));
    });
    return { simpleOpeningArray: openingArray, multiplyDealingArray: dealingArray, simpleTranscriptArray: transcriptArray };
}
export async function _getMasterPublicKey(transcript_key) {
    let generatePublicKeys = await init().then(() => generate_master_public_key_and_address(transcript_key));
...
export async function _combineSignedTransactions(seed, threshold, message, signatureArray, transcript_key, transcript_kappa) {
    ...
    await init().then(async () => {
        try {
            combinedTransactions = await combine_signed_shares(threshold, seed, message, signatureArray, transcript_key, transcript_kappa);
        }
        catch (exceptionVar) {
            alert(exceptionVar);
        }
...
export async function _combineSignedTransactionsWithoutLambda(seed, threshold, message, signatureArray, transcript_key, transcript_kappa) {
    ...
    await init().then(async () => {
        try {
            combinedTransactions = await combine_signed_shares_without_lambda(threshold, seed, message, signatureArray, transcript_key, transcript_kappa);
        }
        catch (exceptionVar) {
            alert(exceptionVar);
        }
...
//this seems to be only run by the original users.
// the new user will be unable to run
...
export async function _reshareStepOneByNewUser(seed, threshold, index, newMegaPkArray) {
    ...
    //pedersenDealingsKappaReshare, pedersenDealingsLambdaReshare
    return { reshareDealings };
    //we cannot modify the old existing megapublickey array in case the reshare fails
}
export async function _reshareStepTwo(seed, threshold_reshare, index, megaPkArray, encryptionSignature, encryptedMegaSecret, simple_dealings_key_reshared_once_or_twice, simpleTranscriptKey, pedersenDealingsKappaReshare, pedersenDealingsLambdaReshare) {
    ...
    await init().then(() => {
        ...
        reshareTranscripts.push(openingReshareAndTranscript[1]); //transcript_list_key_reshared_once
        reshareTranscripts.push(resharePedersenOpening1[1]); //transcriptlistkappa
        reshareTranscripts.push(resharePedersenOpening2[1]); //transcriptlistlambda
        ...
        reshareDealings.push(reshareSimpleDealing1); //simple_dealings_key_reshared_twice
        reshareDealings.push(reshareSimpleDealing2); //simple_dealings_kappa_reshare
    });
...
export async function _reshareStepThree(seed, threshold_reshare, index, megaPkArray, encryptionSignature, encryptedMegaSecret, simpleDealingsKappaReshare, pedersenOpeningLambdaReshareArray, simple_dealings_key_reshared_once_or_twice, transcript_key_simple_or_reshared_once, transcriptKappaReshare) {
    ...
    await init().then(() => {
        ...
        reshareTranscripts.push(openingReshareAndTranscript1[1]); //transcript_key_Reshared_twice
        reshareTranscripts.push(openingReshareAndTranscript2[1]); //transcript_kappa_Reshared_
...
export async function _reshareSignTransaction(seed, threshold_reshare, index, message, encryptionSignature, encryptedMegaSecret, pedersenDealingsLambdaReshare, dealingsKeyXlambdaReshare, // from previous
dealingsKappaXlambdaReshare, // from previous
pedersenOpeningLambdaReshare, transcriptKeyResharedTwice, simpleTranscriptsKappaReshare) {
    let signedTransaction = "";
    await init().then(async () => {
...
export async function _reshareCombineSignedTransactions(simpleTranscriptKappaReshare, transcriptKeyResharedTwice, threshold, seed, message, signatureArray) {
    const combinedTransactions = await init().then(async () => combine_signed_shares_reshare(simpleTranscriptKappaReshare, transcriptKeyResharedTwice, threshold, seed, message, signatureArray));
    return { finalSignedTransaction: combinedTransactions };
}
