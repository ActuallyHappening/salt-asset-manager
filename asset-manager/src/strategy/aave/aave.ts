...
export async function deposit() {
  ...
  const data = contractInterface.encodeFunctionData("depositETH", [
    poolContractAddress,
    vault.masterPublicAddress,
    0,
  ]);
...
export async function approve() {
  ...
  const gas = await aETHWETH.estimateGas.approve(
    WrappedTokenGatewayV3ContractAddress,
    balance,
  );
  ...
  if (approval !== "yes") {
    console.log(
      "the transaction proposal cannot carry on without the signature",
    );
    return;
  }
